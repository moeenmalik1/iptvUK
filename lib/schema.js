...
// Both the homepage and /pricing describe the same thing: one subscription sold
// on four billing terms. They share a single @id so Google treats them as one
// product rather than five competing ones.
const PRODUCT_ID = `${SITE_URL}/#iptv-uk-subscription`;
...
// `review` and `aggregateRating` are only emitted once real customer reviews
// exist in data/reviews.js. Google requires review markup to reflect genuine
// feedback that is also visible on the page, so an empty list has to produce no
// rating fields at all rather than a placeholder.
...
function subscriptionProduct(url) {
...
export function getPricingPageSchema() {
  return {
    '@context': 'https://schema.org',
    '@graph': [
      subscriptionProduct(`${SITE_URL}/pricing`),
      faqPage(pricingFaqs),
